...
object LocationProvider {
    ...
    fun getLocation(context: Context, result: LocationResult?): Boolean {
        ...
        timer1 = Timer()
        timer1!!.schedule(GetLastLocation(), 50000)
        return true
    }
    ...
    internal class GetLastLocation : TimerTask() {
        override fun run() {
            ...
            //if there are both secrets.xml use the latest one
            if (gps_loc != null && net_loc != null) {
                if (gps_loc.time > net_loc.time) locationResult!!.gotLocation(gps_loc) else locationResult!!.gotLocation(net_loc)
                return
            }
            if (gps_loc != null) {
